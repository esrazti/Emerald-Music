...
class ConsoleControlView(ui.View):
    ...
    async def pause_button(self, button: ui.Button, interaction: nextcord.Interaction):
        if self.session.voice_client.is_playing():
            ...
            await interaction.response.send_message("âŒ Nothing playing", ephemeral=True, delete_after=2)
    ...
    async def resume_button(self, button: ui.Button, interaction: nextcord.Interaction):
        if self.session.voice_client.is_paused():
            ...
            await interaction.response.send_message("âŒ Already playing", ephemeral=True, delete_after=2)
    ...
    async def skip_button(self, button: ui.Button, interaction: nextcord.Interaction):
        # Fix: Check for regular song OR background music
        if self.session.current or self.session.is_bg_playing:
            self.session.skip() # This will stop either the song or the BG music
            await interaction.response.send_message("â­ï¸ Skipped", ephemeral=Config.CONSOLE_EPHEMERAL, delete_after=2)
        else:
            await interaction.response.send_message("âŒ Nothing to skip", ephemeral=True, delete_after=2)
    
    @ui.button(label="ðŸ”", style=ButtonStyle.secondary, custom_id="loop_btn")
    async def loop_button(self, button: ui.Button, interaction: nextcord.Interaction):
        ...
        await interaction.response.send_message(f"ðŸ” Loop: {mode_names[next_mode]}", ephemeral=Config.CONSOLE_EPHEMERAL, delete_after=2)
    
    @ui.button(label="ðŸ“»", style=ButtonStyle.secondary, custom_id="radio_btn")
    async def radio_button(self, button: ui.Button, interaction: nextcord.Interaction):
        if not self.session.current:
            await interaction.response.send_message("âŒ Play a song first", ephemeral=True, delete_after=2)
            return
        
        if self.session.radio_mode:
            self.session.disable_radio_mode()
            await interaction.response.send_message("ðŸ“» Radio disabled", ephemeral=Config.CONSOLE_EPHEMERAL, delete_after=2)
        else:
            await self.session.enable_radio_mode(self.session.current.video_id)
            await interaction.response.send_message("ðŸ“» Radio enabled", ephemeral=Config.CONSOLE_EPHEMERAL, delete_after=2)
    
    @ui.button(label="ðŸ”€", style=ButtonStyle.secondary, custom_id="shuffle_btn")
    async def shuffle_button(self, button: ui.Button, interaction: nextcord.Interaction):
        if self.session.queue:
            self.session.shuffle()
            await interaction.response.send_message(f"ðŸ”€ Shuffled {len(self.session.queue)} songs", ephemeral=Config.CONSOLE_EPHEMERAL, delete_after=2)
        else:
            await interaction.response.send_message("âŒ Queue is empty", ephemeral=True, delete_after=2)
    
    @ui.button(label="ðŸ—‘ï¸", style=ButtonStyle.danger, custom_id="clear_btn")
    async def clear_button(self, button: ui.Button, interaction: nextcord.Interaction):
        ...
        await interaction.response.send_message(f"ðŸ—‘ï¸ Cleared {queue_size} songs", ephemeral=Config.CONSOLE_EPHEMERAL, delete_after=2)
    
    @ui.button(label="ðŸ‘‹", style=ButtonStyle.danger, custom_id="leave_btn")
    async def leave_button(self, button: ui.Button, interaction: nextcord.Interaction):
        ...
        await interaction.response.send_message("ðŸ‘‹ Disconnected", ephemeral=Config.CONSOLE_EPHEMERAL, delete_after=2)
    ...
    async def create_source(cls, ctx, search: str, *, loop=None, download=True, is_playlist=False):
        ...
        try:
            ...
            if hasattr(ctx, 'followup'):
                await ctx.followup.send(f"âŒ ã‚¨ãƒ©ãƒ¼: {str(e)[:100]}")
            raise
...
class MusicPlayer:
    ...
    def create_console_embed(self):
        ...
        if self.current and not self.is_bg_playing:
            ...
            embed.add_field(name="ðŸŽµ Now Playing", value=now_playing, inline=False)
        elif self.is_bg_playing:
            embed.add_field(name="ðŸŽµ Now Playing", value="*Background Music â™ª*", inline=False)
        else:
            embed.add_field(name="ðŸŽµ Now Playing", value="*Idle*", inline=False)
        ...
        if self.queue:
            ...
            if len(self.queue) > 5:
                queue_preview += f"\n*...+{len(self.queue) - 5}*"
            embed.add_field(name="ðŸ“œ Queue", value=queue_preview, inline=False)
        else:
            queue_status = "âˆž Radio" if self.radio_mode else "*Empty*"
            embed.add_field(name="ðŸ“œ Queue", value=queue_status, inline=False)
        
        # Status
        state_icon = "â–¶ï¸" if self.voice_client.is_playing() else "â¸ï¸" if self.voice_client.is_paused() else "â¹ï¸"
        loop_icons = {LoopMode.NONE: "âž¡ï¸", LoopMode.SONG: "ðŸ”‚", LoopMode.QUEUE: "ðŸ”", LoopMode.RANDOM: "ðŸ”€"}
        
        status = [
            f"{state_icon} {self.state.value.title()}",
            f"{loop_icons[self.loop_mode]} Loop",
            f"ðŸ”Š {int(self.volume * 100)}%"
        ]
        
        if self.crossfade_enabled:
            status.append(f"ðŸŽµ CF {self.crossfade_duration}s")
        if self.radio_mode:
            status.append("ðŸ“» Radio")
        
        embed.add_field(name="ðŸ“Š Status", value=" â€¢ ".join(status), inline=False)
    ...
    async def enable_radio_mode(self, seed_video_id: str):
        ...
        logger.info(f"ðŸ“» Radio enabled: {seed_video_id}")
    ...
    async def play_bg_music(self):
        ...
        try:
            ...
            logger.info(f"ðŸŽµ Playing BG: {os.path.basename(track)}")
    ...
    def create_now_playing_embed(self, song):
        ...
        if self.radio_mode:
            embed.title = "ðŸ“» Radio Mode â€¢ Now Playing"
        else:
        ...
        embed.add_field(name="ðŸ”Š Volume", value=f"`{int(self.volume * 100)}%`", inline=True)
        
        queue_info = f"âˆž (Radio)" if self.radio_mode else f"`{len(self.queue)}`"
        embed.add_field(name="ðŸ“œ Queue", value=queue_info, inline=True)
        
        if self.crossfade_enabled:
            embed.add_field(name="ðŸŽµ Crossfade", value=f"`{self.crossfade_duration}s`", inline=True)
    ...
    def create_queue_embed(self):
        if self.radio_mode:
            embed = Embed(title="ðŸ“» Radio Queue", color=Config.THEME_SUB)
        else:
            embed = Embed(title="ðŸ“œ Music Queue", color=Config.THEME_MAIN)
        ...
        duration_str = str(timedelta(seconds=total)) if total else "âˆž" if self.radio_mode else "0:00"
        
        footer = f"ðŸ“Š {len(self.queue)} songs â€¢ â±ï¸ {duration_str} â€¢ ðŸ” {self.loop_mode.value}"
...
class MusicBot(commands.Bot):
    ...
    async def on_ready(self):
        ...
        logger.info(f'ðŸ“Š Servers: {len(self.guilds)}')
        logger.info(f'ðŸŽµ BG Music: {len(self.bg_manager.bg_tracks)} tracks')
...
async def join(ctx: nextcord.Interaction):
    if not ctx.user.voice:
        return await ctx.send("âŒ You need to be in a voice channel!", ephemeral=True)
    ...
    if ctx.guild.voice_client:
        ...
        if Config.ENABLE_CONSOLE_MODE:
            ...
            embed.set_footer(text="ðŸŽ›ï¸ Console created")
...
async def leave(ctx: nextcord.Interaction):
    if not ctx.guild.voice_client:
        return await ctx.send("âŒ Not connected!", ephemeral=True)
    
    await bot.session_manager.destroy_session(ctx.guild.id)
    embed = Embed(description="ðŸ‘‹ Disconnected", color=Config.THEME_SUB)
    await ctx.send(embed=embed)
...
async def console(ctx: nextcord.Interaction, ephemeral: bool = SlashOption(required=False, default=False)):
    ...
    if not session:
        return await ctx.send("âŒ No active session!", ephemeral=True)
...
async def play(ctx: nextcord.Interaction, search: str = SlashOption(description="Song name or URL", required=True)):
    ...
    if not ctx.user.voice:
        return await ctx.followup.send("âŒ You need to be in a voice channel!", ephemeral=True)
    ...
    try:
        ...
        if not sources:
            return await ctx.followup.send("âŒ No results found!")
        ...
        if len(added) == 1:
            ...
            embed.add_field(name="ðŸ“", value=f"`#{len(session.queue)}`", inline=True)
        else:
            embed = Embed(color=Config.THEME_MAIN)
        ...
        await ctx.followup.send(f"âŒ Error: {str(e)[:100]}")
...
async def radio(ctx: nextcord.Interaction):
    ...
    if not session or not session.current:
        return await ctx.followup.send("âŒ Play a song first!", ephemeral=True)
    
    await session.enable_radio_mode(session.current.video_id)
    
    embed = Embed(color=Config.THEME_SUB)
    embed.title = "ðŸ“» Radio Mode Activated"
...
async def crossfade(ctx: nextcord.Interaction, duration: float = SlashOption(required=False, min_value=0.0, max_value=10.0)):
    ...
    if duration is None:
        ...
        embed = Embed(description=f"ðŸŽµ Crossfade: **{status}**", color=Config.THEME_MAIN)
    elif duration == 0:
        session.crossfade_enabled = False
        embed = Embed(description="ðŸŽµ Crossfade: **disabled**", color=Config.THEME_SUB)
    else:
        session.crossfade_enabled = True
        session.crossfade_duration = duration
        embed = Embed(description=f"ðŸŽµ Crossfade: **{duration}s**", color=Config.THEME_MAIN)
...
async def pause(ctx: nextcord.Interaction):
    session = bot.session_manager.get_session(ctx.guild.id)
    if not session:
        return await ctx.send("âŒ Nothing playing!", ephemeral=True)
    session.pause()
...
async def loop(ctx: nextcord.Interaction, mode: str = SlashOption(choices=["off", "song", "queue", "random"], required=True)):
    ...
    await ctx.send(embed=Embed(description=f"ðŸ” Loop: **{mode}**", color=Config.THEME_MAIN))
...
async def volume(ctx: nextcord.Interaction, level: int = SlashOption(min_value=0, max_value=100, required=True)):
    ...
    icon = "ðŸ”‡" if level == 0 else "ðŸ”‰" if level < 30 else "ðŸ”Š"
...
async def shuffle(ctx: nextcord.Interaction):
    ...
    if not session or not session.queue:
        return await ctx.send("âŒ Queue is empty!", ephemeral=True)
    session.shuffle()
    await ctx.send(embed=Embed(description=f"ðŸ”€ Shuffled {len(session.queue)} songs", color=Config.THEME_SUB))
...
async def clear(ctx: nextcord.Interaction):
    ...
    session.clear_queue()
    await ctx.send(embed=Embed(description=f"ðŸ—‘ï¸ Cleared {size} songs", color=Config.THEME_SUB))
...
async def help_command(ctx: nextcord.Interaction):
    embed = Embed(title="ðŸŽµ Music Bot Commands", color=Config.THEME_MAIN)
...
async def playlist_create(ctx: nextcord.Interaction, name: str = SlashOption(required=True)):
    ...
    if bot.playlist_manager.create_playlist(ctx.user.id, name, creator):
        ...
        await ctx.send(f"âŒ Playlist **{name}** already exists!", ephemeral=True)
...
async def playlist_add(ctx: nextcord.Interaction, name: str = SlashOption(required=True), song: str = SlashOption(required=False)):
    ...
    if not song:
        session = bot.session_manager.get_session(ctx.guild.id)
        if not session or not session.current:
            return await ctx.followup.send("âŒ Play a song or specify a URL!", ephemeral=True)
        ...
        if bot.playlist_manager.add_to_playlist(ctx.user.id, name, song_data):
            ...
            await ctx.followup.send(f"âŒ Playlist **{name}** not found!", ephemeral=True)
    else:
        try:
            sources = await AudioSource.create_source(ctx, song, loop=bot.loop, download=False)
            
            if not sources:
                return await ctx.followup.send("âŒ No results found!", ephemeral=True)
            ...
            if added:
                ...
                await ctx.followup.send(f"âŒ Playlist not found!", ephemeral=True)
        except Exception as e:
            await ctx.followup.send(f"âŒ Error: {str(e)[:100]}", ephemeral=True)
...
async def playlist_play(ctx: nextcord.Interaction, name: str = SlashOption(required=True)):
    ...
    if not songs:
        return await ctx.followup.send(f"âŒ Playlist **{name}** not found!", ephemeral=True)
    
    if not ctx.guild.voice_client:
        if not ctx.user.voice:
            return await ctx.followup.send("âŒ You need to be in a voice channel!")
    ...
    embed = Embed(color=Config.THEME_MAIN)
    embed.title = "ðŸ“ Playing Playlist"
    embed.description = f"**{name}**"
...
async def playlist_list(ctx: nextcord.Interaction):
    ...
    if not playlists:
        embed = Embed(
            description="ðŸ“ You don't have any playlists yet!\nUse `/playlist_create` to make one.",
            color=Config.THEME_SUB
        )
        return await ctx.send(embed=embed, ephemeral=True)
    
    embed = Embed(title="ðŸ“ Your Playlists", color=Config.THEME_MAIN)
...
async def playlist_delete(ctx: nextcord.Interaction, name: str = SlashOption(required=True)):
    if bot.playlist_manager.delete_playlist(ctx.user.id, name):
        ...
        await ctx.send(f"âŒ Playlist **{name}** not found!", ephemeral=True)
...
async def ping(ctx: nextcord.Interaction):
    ...
    embed = Embed(title="ðŸ“ Pong!", color=color)
...
async def stats(ctx: nextcord.Interaction):
    ...
    embed = Embed(title="ðŸ“Š Bot Statistics", color=Config.THEME_MAIN)
    embed.add_field(name="ðŸŒ Servers", value=f"`{len(bot.guilds)}`", inline=True)
    embed.add_field(name="ðŸŽµ Sessions", value=f"`{len(bot.session_manager.sessions)}`", inline=True)
    embed.add_field(name="â±ï¸ Uptime", value=f"`{hours}h {minutes}m`", inline=True)
    embed.add_field(name="ðŸ“ Latency", value=f"`{round(bot.latency * 1000)}ms`", inline=True)
    embed.add_field(name="ðŸ’¾ Cache", value=f"`{cache_mb:.1f} MB`", inline=True)
    embed.add_field(name="ðŸŽµ BG Tracks", value=f"`{len(bot.bg_manager.bg_tracks)}`", inline=True)
    
    features = []
    features.append(f"ðŸŽ›ï¸ Console: {'âœ…' if Config.ENABLE_CONSOLE_MODE else 'âŒ'}")
    features.append(f"ðŸŽµ Crossfade: {'âœ…' if Config.ENABLE_CROSSFADE else 'âŒ'} ({Config.CROSSFADE_DURATION}s)")
    features.append(f"ðŸ“» BG Music: {'âœ…' if Config.ENABLE_BG_MUSIC else 'âŒ'}")
...
async def tts(ctx: nextcord.Interaction, text: str = SlashOption(required=True), language: str = SlashOption(choices=["en", "ja", "th", "ko", "zh"], default="en")):
    ...
    try:
        ...
        embed = Embed(color=Config.THEME_MAIN)
        embed.description = f"ðŸ”Š TTS: *{text[:80]}{'...' if len(text) > 80 else ''}*"
        await ctx.followup.send(embed=embed)
        ...
        try:
            await ctx.followup.send(f"âŒ TTS error: {e}")
        except:
            await ctx.send(f"âŒ TTS error: {e}", ephemeral=True)
...
async def forcedisconnect(ctx: nextcord.Interaction):
    ...
    embed = Embed(description=f"ðŸ”Œ Disconnected from {count} sessions", color=Config.THEME_SUB)
...
async def on_application_command_error(ctx: nextcord.Interaction, error: Exception):
    if isinstance(error, commands.CommandOnCooldown):
        ...
        await ctx.send("âŒ Missing permissions!", ephemeral=True)
    elif isinstance(error, commands.BotMissingPermissions):
        await ctx.send("âŒ Bot missing permissions!", ephemeral=True)
    else:
        logger.error(f"Error: {error}", exc_info=True)
        error_embed = Embed(description=f"âŒ Error:\n```{str(error)[:200]}```", color=Config.THEME_BG)
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("ðŸŽµ Discord Music Bot Starting...")
    logger.info("âœ¦ ã‚¨ãƒ¡ãƒ©ãƒ«ãƒ‰ âœµ ãƒŸãƒ¥ãƒ¼ã‚¸ãƒƒã‚¯ âœ¦ Emerald Music")
    logger.info("=" * 60)
    logger.info(f"ðŸ“Š Preload: {Config.PRELOAD_COUNT} songs")
    logger.info(f"ðŸ“» Radio Preload: {Config.RADIO_PRELOAD} songs")
    logger.info(f"ðŸ’¾ RAM Limit: {Config.MAX_RAM_MB}MB")
    logger.info(f"ðŸŽ›ï¸ Console: {Config.ENABLE_CONSOLE_MODE} (Refresh: {Config.CONSOLE_REFRESH_RATE}s)")
    logger.info(f"ðŸŽµ Crossfade: {Config.ENABLE_CROSSFADE} ({Config.CROSSFADE_DURATION}s)")
    logger.info(f"ðŸ“» BG Music: {Config.ENABLE_BG_MUSIC} (Delay: {Config.BG_MUSIC_IDLE_DELAY}s)")
    logger.info(f"ðŸŽµ BG Music Tracks Found: {len(bot.bg_manager.bg_tracks)}")
    logger.info(f"ðŸŽ¨ Theme: Elegant Blue & Purple")
    logger.info("=" * 60)
    
    from web_server import start_web_server
    start_web_server(bot ,   host='0.0.0.0', port=5000, 
                # use_ssl=True, 
                # cert_path='ssl_certs/cert.pem',
                # key_path='ssl_certs/key.pem'
                 )
